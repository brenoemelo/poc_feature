...
- **Rule:** Use `Microsoft.Extensions.Logging` (native `ILogger`). We have replaced Serilog to reduce complexity and improve performance.
...
3. **Is the App pointing to the Collector?**
   - Check environment variables or `appsettings.json`: `"Otel": { "Endpoint": "http://localhost:4317" }`.
