...
def request_exists(name, parent_id):
    return any(
        r
        for r in resources
        if r["_type"] == "request" and r["name"] == name and r["parentId"] == parent_id
    )
...
# 5. DataHelper - Get Version
if workspace_id and not any(
    r for r in resources if r["_id"] == "fld_datahelper" and r["_type"] == "request_group"
):
    new_resources.append(
        {
            "_id": "fld_datahelper",
            ...
            "modified": current_time,
            "created": current_time,
            "name": "DataHelper Service",
            "description": "Endpoints for helper utilities (build version, etc).",
            "environment": {},
            "environmentPropertyOrder": None,
            "metaSortKey": -1707868400000,
            "_type": "request_group",
        }
    )

if not request_exists("Get DataHelper Version", "fld_datahelper"):
    new_resources.append(
        {
            "_id": "req_datahelper_get_version",
            "parentId": "fld_datahelper",
            "modified": current_time,
            "created": current_time,
            "url": "{{ _.base_url }}/api/v1/datahelper/version",
            "name": "Get DataHelper Version",
            "description": "Retrieve build InformationalVersion for PoC.DataHelper.",
            "method": "GET",
            "body": {},
            "parameters": [],
            "headers": [],
            "authentication": {},
            "metaSortKey": -1707868390000,
            "isPrivate": False,
            "settingStoreCookies": True,
            "settingSendCookies": True,
            "settingDisableRenderRequestBody": False,
...
if new_resources:
    ...
    print(f"Added {len(new_resources)} new resources.")
else:
